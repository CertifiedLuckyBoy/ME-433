...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define TX_ID          (0x433)   /* HW12 CAN message identifier */
#define RX_ID          (0x433)
/* USER CODE END PD */
...
uint8_t rxData[16U];
static const uint8_t txData[] = {'K', 'A', 'R', 'L', '-', 'H', 'W', '1',
                                 '2', '-', 'C', 'A', 'N', '!', '!', '!'};
static volatile uint8_t txPrintPending = 0U;
...
static void PrintCanMessage(const char *direction, uint32_t identifier, const uint8_t *data, uint16_t length);
...
int main(void)
{
  ...
  /* Keep the User push-button in interrupt mode for CAN transmission. */
  BSP_PB_Init(BUTTON_USER, BUTTON_MODE_EXTI);
  ...
  printf("\r\nHW12 FDCAN ready: ID=0x%03lX, press USER to send\r\n",
         (unsigned long)TX_ID);
  ...
  while (1)
  {
    ...
    {
      if (txPrintPending != 0U)
      {
        txPrintPending = 0U;
        PrintCanMessage("TX", TX_ID, txData, COUNTOF(txData));
      }
    }
    ...
    {
      PrintCanMessage("RX", rxHeader.Identifier, rxData, COUNTOF(rxData));
...
static void PrintCanMessage(const char *direction, uint32_t identifier, const uint8_t *data, uint16_t length)
{
  ...
  printf("%s ID=0x%03lX DATA=\"", direction, (unsigned long)identifier);
  for (index = 0U; index < length; index++)
  {
    printf("%c", (data[index] >= 32U && data[index] <= 126U) ? data[index] : '.');
  }
  printf("\" HEX=");
  for (index = 0U; index < length; index++)
  {
    printf("%02X%s", data[index], (index + 1U < length) ? " " : "");
  }
  printf("\r\n");
}
/* USER CODE END 4 */
